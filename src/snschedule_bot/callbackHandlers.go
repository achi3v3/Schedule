...
func UniversalHandler(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if callbackData == "Закрепить группу" {
		if state["course"] != "" && state["group"] != "" {
			answer, err := PinGroup(ctx, userID, state["course"], state["group"])
			if err == nil {
				b.SendMessage(ctx, &bot.SendMessageParams{
					ChatID:    chatID,
					Text:      answer,
			...
			b.SendMessage(ctx, &bot.SendMessageParams{
				ChatID: chatID,
				Text:   "💢 Пожалуйста, выберите курс и группу перед закреплением.",
			})
		}
		return
	}
